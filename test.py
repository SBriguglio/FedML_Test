"""
Sourced directly from the FedML repository:
https://github.com/FedML-AI/FedML/blob/master/fedml_experiments/standalone/fedavg/main_fedavg.py
Some annotations, signed with "SB" added by Spencer Briguglio
For brevity, I have removed lines which are not relevant to the FEMNIST example. However, I have not changed the
structure or function of the code to any considerable degree. This way, you can trace the code as it works in the
FedML repository's example (source above), while not being distracted by irrelevant lines.
"""
...
# Parse arguments from test.sh -SB
def add_args(parser):
    """
